    ...
    <>
      <div className={`menu ${menuOpen ? "open" : ""}`}>
        <div className="menu-container">
          <svg className="close-menu" onClick={closeMenu} width="30px" height="30px" viewBox="0 0 1024.00 1024.00" fill="#ffffff" stroke="#ffffff">
        ...
        <div className="flex flex-col justify-center items-center gap-5 text-sm font-medium font-jetbrains">
          ...
          <Link className="text-white text-2xl hover:text-green-500" href="/projects" onClick={closeMenu}>{t('projects')}</Link>
        </div>
        <div></div>
      </div>
      ...
      <header className="w-full bg-primary lg:px-32 lg:py-5 fixed top-0 z-10 ">
        ...
        <div className="flex md:hidden container py-5 items-center justify-between px-5">
          <div className={`container-svg-header ${menuOpen ? "open" : ""}`}>
            <svg onClick={toggleMenu} width="35px" height="35px" viewBox="0 -1 12 12" fill="none">
